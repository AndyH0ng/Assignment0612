...
void draw() {
  ...
  if (isBabyRotating) {
    ...
    if (miniIndex < miniImages.length) {
      image(miniImages[miniIndex], width/2, babyY, babyWidth, babyHeight);
      ...
      // dangun 이미지 출력
      image(dangun, width/2, babyY, babyWidth, babyHeight);
      isDangunDisplayed = true; // dangun 이미지가 출력되었음을 표시
    }
  } else {
    // 베이비 이미지 그리기
    image(isBaby1 ? baby : baby2, width/2, babyY, babyWidth, babyHeight);
